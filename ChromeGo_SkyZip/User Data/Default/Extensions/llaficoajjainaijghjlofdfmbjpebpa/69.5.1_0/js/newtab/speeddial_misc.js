(function(){
  ...
  SpeedDialMisc.prototype = {
    ...
    doSearch: function(query) {
      query = query || document.getElementById("q").value;

      if(String(query).trim().length == 0){ // Task #1875
        console.info('Empty string');
        return false;
      }

      /*
      if(fvdSpeedDial.Utils.getInstallVersion() < 6930){
        var url = "http://fvdmedia.com/addon_search/?q=" + encodeURIComponent(query) +
        "&from=chrome_fvdsd" +
        "&install_time=" + fvdSpeedDial.Prefs.get("sd.install_time");
      }else{
        var url = 'http://searchingtraining.com/?a=gsb_lsphp_00_00_ssg01&q='+encodeURIComponent(query); // #2098
      }
      */
      ...
      var url = "http://fvdmedia.com/addon_search/?q=" + encodeURIComponent(query) +
        "&from=chrome_fvdsd" +
        "&install_time=" + fvdSpeedDial.Prefs.get("sd.install_time") +
        "&search_type=" + ( fvdSpeedDial.Utils.getInstallVersion() < 6930 ? '0' : '1' )
      ;
